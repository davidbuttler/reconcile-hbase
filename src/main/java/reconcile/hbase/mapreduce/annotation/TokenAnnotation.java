...
import reconcile.hbase.mapreduce.annotation.SentenceAnnotation.SentenceMapper;
  ...
  try {
    ...
    if (toks == null || toks.size() == 0) {
      ...
      for (Annotation sent : sentences) {
        ...
        String sentText = Utils.getAnnotText(sent, rawText);
    ...
    throws IOException, InterruptedException
{
  ...
  while (true) {
    try {
      ...
      context1.getCounter("Token Annotation error", "io exception in flush/close").increment(1);
      e.printStackTrace();
    }
    break;
  }
