...
public class SizeColumnFamily
    extends Configured
    implements Tool {

private static final Log LOG = LogFactory.getLog(SizeColumnFamily.class);
...
{
  try {
    ToolRunner.run(new Configuration(), new SizeColumnFamily(), args);
  }
  catch (Exception e) {
    e.printStackTrace();
  }
    ...
    throws Exception
{
  ...
  try {
    ...
    job.setJarByClass(SizeColumnFamily.class);
